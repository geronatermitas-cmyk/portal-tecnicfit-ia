...
// Usa un modelo que SÍ tienes en /v1/models
...
async function callGemini(body: unknown) {
  ...
  if (!resp.ok) return { ok: false, status: 502, body: { error: 'Upstream error', status: resp.status, details: text } };

  try { return { ok: true, status: 200, body: JSON.parse(text) }; }
  catch { return { ok: true, status: 200, body: { raw: text } }; }
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (action === 'generateImageForTerm') return send(res, 501, { error: 'Image generation not implemented' });

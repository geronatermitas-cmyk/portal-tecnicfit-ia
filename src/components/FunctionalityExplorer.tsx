...
// Iconos (lucide-react) para heurística visual
import {
  Mic,
  Languages,
  Volume2,
...
function pickIconByName(name: string): LucideIcon {
  ...
  if (n.includes('escucha')) return Headphones;
  return BadgeInfo;
}
...
const FunctionalityCard: React.FC<{ item: Functionality; onSpeak: (t: string) => void }> = ({ item, onSpeak }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-between rounded-2xl border border-gray-200 bg-white p-6 text-center shadow-sm transition hover:shadow-md dark:border-gray-700 dark:bg-gray-800">
      ...
      <p className="mt-2 line-clamp-3 text-sm text-gray-600 dark:text-gray-300">{item.descripcion}</p>
...
export const FunctionalityExplorer: React.FC<FunctionalityExplorerProps> = ({ category, goBack }) => {
  ...
  const pretty = {
    visual:   { t: 'Discapacidad Visual',   d: 'Funcionalidades y software útiles para discapacidad visual.' },
    auditiva: { t: 'Discapacidad Auditiva', d: 'Funcionalidades y software útiles para discapacidad auditiva.' },
    habla:    { t: 'Discapacidad del Habla', d: 'Funcionalidades y software útiles para trastornos del habla.' },
  }[category];

  useSEO({
    title: `Funcionalidades · ${pretty.t} · Portal de Accesibilidad`,
    description: `${pretty.d} Listados por plataformas y con lectura por voz.`,
    url: `${origin}/?view=functionalities&category=${category}`,
    jsonLd: {
      '@context': 'https://schema.org',
      '@type': 'CollectionPage',
      name: `Funcionalidades · ${pretty.t} · Portal de Accesibilidad`,
      url: `${origin}/?view=functionalities&category=${category}`,
      inLanguage: 'es',
      about: pretty.d,
    ...
    const load = async () => {
      try {
        ...
        const normalized = base.map(f => ({
          ...
          imageUrl: '', // por si en el futuro añadimos imágenes
        }));
